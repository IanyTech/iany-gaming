...
function initMobileDrawer() {
  ...
  drawer.addEventListener('click', (e) => {
    ...
    // Handle actions (account/login/register)
    if (t.matches('[data-action="account"]')) {
      try { setTimeout(()=> location.hash = '#account', 0); } catch(_) {}
      setOpen(false);
    }
...
// Account Auth Modal controller (password-based)
function setupAccountAuthModal() {
  ...
  const showPwEl = document.getElementById('authShowPw');
  ...
  const setMode = (mode) => {
    ...
    if (help) help.textContent = mode === 'login'
      ? 'Accedi con email e password.'
      : 'Crea un account con email e password.';
    if (pwEl) pwEl.autocomplete = mode === 'register' ? 'new-password' : 'current-password';
    // Clear any previous validation message when switching mode
    try { pwEl.setCustomValidity(''); } catch(_) {}
  };
  ...
  window.openAuth = (mode = 'login') => { setMode(mode); show(); };
  window.closeAuth = () => { hide(); };
  // Back-compat for any existing callers
  window.openAccountAuth = (mode = 'login') => { try { window.openAuth(mode); } catch(_) {} };
  window.closeAccountAuth = () => { try { window.closeAuth(); } catch(_) {} };

  closeBtn && (closeBtn.onclick = () => window.closeAuth());
  cancelBtn && (cancelBtn.onclick = () => window.closeAuth());
  tabLogin && (tabLogin.onclick = () => setMode('login'));
  tabRegister && (tabRegister.onclick = () => setMode('register'));

  showPwEl && (showPwEl.onchange = () => { try { pwEl.type = showPwEl.checked ? 'text' : 'password'; } catch(_) {} });

  // Password policy validation (live)
  const validatePw = (pw) => {
    const reasons = [];
    ...
    pwEl.addEventListener('input', () => {
      try {
        if (modal.dataset.mode === 'register') {
          const { ok, reasons } = validatePw(pwEl.value || '');
          pwEl.setCustomValidity(ok ? '' : 'Password non conforme: ' + reasons.join(', '));
        } else {
          pwEl.setCustomValidity('');
        }
      } catch(_) {}
    });
  }
  ...
  forgotBtn && (forgotBtn.onclick = async () => {
    ...
    if (!(supaReady && supaReady())) { showToast?.('Recupero password non disponibile', 'error'); return; }
    try {
      const redirectTo = location.origin + '#account';
      const { error } = await supabase.auth.resetPasswordForEmail(email, { redirectTo });
      if (error) throw error;
      showToast?.('Email per reimpostare la password inviata', 'success');
    } catch (e) {
  ...
  submitBtn && (submitBtn.onclick = async () => {
    ...
    if (!email || !email.includes('@')) { showToast?.('Inserisci una email valida', 'error'); return; }
    if (!password || password.length < 6) { showToast?.('Password troppo corta (min 6 caratteri)', 'error'); return; }
    const mode = modal.dataset.mode === 'register' ? 'register' : 'login';
    if (!(supaReady && supaReady())) { showToast?.('Autenticazione non disponibile', 'error'); return; }
    try {
      if (mode === 'register') {
        // Enforce password policy before attempting signup
        const { ok, reasons } = validatePw(password);
        if (!ok) { showToast?.('Password non conforme: ' + reasons.join(', '), 'error'); pwEl?.reportValidity?.(); return; }
      ...
      if (msg.includes('invalid login') || msg.includes('invalid credentials')) {
        ...
        showToast?.('Utente già registrato, prova ad accedere', 'info');
        setMode('login');
      } else {
        showToast?.('Errore autenticazione', 'error');
      }
    }
  });
  ...
  const ov = document.getElementById('overlay');
  if (ov) {
    ov.addEventListener('click', () => { if (!modal.hidden) window.closeAuth(); });
  }

  // Close on ESC
  document.addEventListener('keydown', (e) => { if (e.key === 'Escape' && !modal.hidden) window.closeAuth(); });

  // Draggable dialog via header (kept like previous)
  ...
  const onDrag = (e) => { if (!dragging) return; const dx = e.clientX - startX; const dy = e.clientY - startY; dialog.style.left = (origLeft + dx) + 'px'; dialog.style.top = (origTop + dy) + 'px'; };
  const endDrag = () => { if (!dragging) return; dragging = false; document.body.style.userSelect = ''; };
...
const I18N = {
  ...
  en: {
    ...
    nav_fav: 'Favorites',
    nav_profile: 'Profile',
    hero_title: 'Welcome to Iany Gaming',
...
function initSettingsSection() {
  ...
  scope.addEventListener('click', (e) => {
    ...
    if (t.matches('.chip')) {
      ...
      applySettingsFromStore();
      syncActive();
      if (key === 'currency' || key === 'locale' || key === 'lang' || key === 'showdiscounts' || key === 'accent' || key === 'corners' || key === 'view' || key === 'vat' || key === 'rounding' || key === 'effects') {
...
const PRODUCTS = [
  ...
  { id: 'Steam Gift Card 10€', name: 'Carta Regalo Steam ', price: 9.00, old: 10.00, img: 'assets/grafica5.png', tag: 'console' },
  { id: 'Steam Gift Card 20€', name: 'Carta Regalo Steam ', price: 23.50, old: 25.00, img: 'assets/grafica5.png', tag: 'console' },
  { id: 'Steam Gift Card 50€', name: 'Carta Regalo Steam ', price: 47.50, old: 50.00, img: 'assets/grafica5.png', tag: 'console' },
...
const PRODUCT_DETAILS = {
  'Xbox Gift Card 10€': {
    desc: 'Gift Card Xbox da 10 euro (Formato Digitale)',
    images: [
      'assets/grafica1.png',
  ...
  'Xbox Gift Card 20€': {
    desc: 'Gift Card Xbox da 20 euro (Formato Digitale)',
    images: [
      'assets/grafica1.png',
  ...
  'Xbox Gift Card 50€': {
    desc: 'Gift Card Xbox da 50 euro (Formato Digitale)',
    images: [
      'assets/grafica1.png',
  ...
  'Playstation Gift Card 10€': {
    desc: 'Gift Card Playstation da 10 euro (Formato Digitale)',
    images: [
      'assets/grafica2.png',
  ...
  'Playstation Gift Card 20€': {
    desc: 'Gift Card Playstation da 20 euro (Formato Digitale) ',
    images: [
      'assets/grafica2.png',
  ...
  'Playstation Gift Card 50€': {
    desc: 'Gift Card Playstation da 50 euro (Formato Digitale)',
    images: [
      'assets/grafica2.png',
  ...
  'Steam Gift Card 10€ ': {
    desc: 'Gift Card Steam da 10 euro (Formato Digitale) ',
    images: [
      'assets/grafica5.png',
  ...
  'Steam Gift Card 25€': {
    desc: 'Gift Card Steam da 25 euro (Formato Digitale)',
    images: [
      'assets/grafica5.png',
  ...
  'Steam Gift Card 50€': {
    desc: 'Gift Card Steam da 50 euro (Formato Digitale)',
    images: [
      'assets/grafica5.png',
    ...
    desc: 'Gift Card 1000 Valorant Points (Formato Digitale) ',
    images: [
      'assets/grafica4.png',
    ...
    desc: 'Gift Card 2050 Valorant Points (Formato Digitale)',
    images: [
      'assets/grafica4.png',
    ...
    desc: 'Gift Card 5350 Valorant Points (Formato Digitale) ',
    images: [
      'assets/grafica4.png',
      ...
      '1000 V-bucks': {
    desc: 'Gift Card  da 1000 V-bucks (Formato Digitale)',
    images: [
      'assets/grafica1.png',
      ...
      '2800 V-bucks': {
    desc: 'Gift Card da 2800  V-bucks (Formato Digitale)',
    images: [
      'assets/grafica2.png',
      ...
      '5000 V-bucks': {
    desc: 'Gift Card da 5000 V-bucks (Formato Digitale)',
    images: [
      'assets/grafica3.png',
         ...
         '13500 V-bucks': {
    desc: 'Gift Card da 13500 V-bucks (Formato Digitale)',
    images: [
      'assets/grafica6.png',
    ]
      },
