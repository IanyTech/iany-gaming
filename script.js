...
function setupAccountAuthModal() {
  ...
  submitBtn && (submitBtn.onclick = async () => {
    ...
    // Supabase removed: simple local session login
...
async function getUserKey() {
  ...
  try { const u = await getSupabaseUser(); const e = (u?.email||'').trim().toLowerCase(); if (e) return 'user:' + e; } catch(_) {}
  try { const s = await getSettings(); const e = (s?.email||'').trim().toLowerCase(); if (e) return 'user:' + e; } catch(_) {}
  return 'anon:guest';
}
...
async function renderCheckout() {
  ...
  try {
    ...
    const loggedIn = !!(u && u.email);
    const prof = readProfile();
    const nameFromAcc = (s?.name || prof?.name || '').trim();
    const emailFromAcc = (u?.email || prof?.email || '').trim();
...
async function renderUser() {
  const u = await getSupabaseUser();
  const email = u?.email || getSessionUser()?.email || '';
...
async function renderAccount() {
  ...
  if (btnSwitch) {
    btnSwitch.onclick = async () => {
      ...
      // After sign-out, open modal to log into another account (fallback to generic auth)
