...
function setupAccountAuthModal() {
  ...
  const setMode = (mode) => {
    ...
    const confirmRow = document.getElementById('accAuthConfirmRow');
    if (title) title.textContent = mode === 'login' ? 'Accedi' : 'Registrati';
    if (help) help.textContent = 'Inserisci email e password per ' + (mode === 'login' ? 'accedere.' : 'registrarti.');
    if (confirmRow) confirmRow.hidden = (mode !== 'register');
  };
  ...
  submitBtn && (submitBtn.onclick = async () => {
    ...
    const pwdEl = document.getElementById('accAuthPassword');
    const pwd2El = document.getElementById('accAuthPassword2');
    const email = (emailEl?.value || '').trim();
    const password = (pwdEl?.value || '').toString();
    const mode = modal.dataset.mode || 'login';
    if (!email || !email.includes('@')) { showToast?.('Inserisci una email valida', 'error'); return; }
    if (!password || password.length < 6) { showToast?.('La password deve avere almeno 6 caratteri', 'error'); return; }
    try {
      if (typeof window !== 'undefined' && window.supabase) {
        if (mode === 'register') {
          const password2 = (pwd2El?.value || '').toString();
          if (password !== password2) { showToast?.('Le password non coincidono', 'error'); return; }
          const { error } = await supabase.auth.signUp({ email, password });
          if (error) throw error;
          showToast?.('Registrazione completata. Ora sei connesso.', 'success');
        } else {
          const { error } = await supabase.auth.signInWithPassword({ email, password });
          if (error) throw error;
          showToast?.('Accesso effettuato', 'success');
        }
        // Sessione aggiornata dal listener; chiudi modale e aggiorna UI
        window.closeAccountAuth();
        renderUser?.();
        renderAccount?.();
        return;
      ...
      renderUser?.();
      showToast?.('Accesso effettuato (locale)', 'success');
      window.closeAccountAuth();
      renderAccount?.();
    } catch (err) {
...
// Supabase removed: disable external functions base
...
// Auth session persistence (prefers Supabase when available)
...
// Legacy compatibility: some parts still call getSupabaseUser
// Map it to the new currentUser() implementation
...
async function initSupabaseAuth() {
  ...
  try {
    const { data: { session } } = await supabase.auth.getSession();
    if (session?.user) setSessionUser(supaUserToSession(session.user));
    if (session?.user) {
      try { await supaBootstrapUserState(); } catch (_) {}
    }
  } catch (_) {}
  try {
    supabase.auth.onAuthStateChange(async (_event, session) => {
      ...
      // On login, try migrating local favorites/cart to Supabase
      if (session?.user) {
        try { await migrateLocalToSupabaseOnLogin(); } catch (_) {}
        try { await supaBootstrapUserState(); } catch (_) {}
      }
      renderUser?.();
      renderAccount?.();
...
// App init (no Supabase listeners)
...
// Supabase email login removed

// Reviews storage (Supabase + fallback)
...
async function supaAvailable() { return (typeof window !== 'undefined' && !!window.supabase); }
async function isLoggedIn() { return !!(await currentUserId()); }

// ----- Favorites (Supabase-backed when logged) -----
async function supaGetFavsSafe() {
  if (!supaAvailable() || !(await isLoggedIn())) return null;
  try {
    const uid = await currentUserId();
    // Expect favorites(user_id, product_id)
...
async function supaAddFavSafe(productId) {
  if (!supaAvailable() || !(await isLoggedIn())) return false;
  try {
    const uid = await currentUserId();
    await supabase.from('favorites').insert({ user_id: uid, product_id: productId });
    return true;
  } catch (_) { return false; }
}
async function supaRemoveFavSafe(productId) {
  if (!supaAvailable() || !(await isLoggedIn())) return false;
  try {
    const uid = await currentUserId();
    await supabase.from('favorites').delete().eq('user_id', uid).eq('product_id', productId);
    return true;
  } catch (_) { return false; }
}
...
function cartDB() {
  return {
    read: () => readLS(LS_KEYS.cart, []),
    write: (list) => writeLS(LS_KEYS.cart, list || []),
  };
}
function favsDB() {
  return {
    read: () => readLS(LS_KEYS.favs, []),
    write: (list) => writeLS(LS_KEYS.favs, list || []),
  };
}

async function toggleFav(id) {
  if (supaAvailable() && await isLoggedIn()) {
    // try Supabase
    const favs = await supaGetFavsSafe();
    if (Array.isArray(favs)) {
      const set = new Set(favs);
      if (set.has(id)) { await supaRemoveFavSafe(id); set.delete(id); }
      else { await supaAddFavSafe(id); set.add(id); }
      writeLS(LS_KEYS.favs, Array.from(set));
      renderFavsIconStates();
      return;
    }
  }
  const db = favsDB();
  const list = new Set(db.read());
  if (list.has(id)) list.delete(id); else list.add(id);
  db.write(Array.from(list));
  renderFavsIconStates();
}
...
async function addToCart(id, qty = 1) {
  if (supaAvailable() && await isLoggedIn()) {
    try {
      const cartId = await supaEnsureActiveCart();
      await supaUpsertCartItem(cartId, id, Math.max(1, qty|0), false);
      renderCart();
      return;
    } catch (_) { /* fallback */ }
  }
...
async function removeFromCart(id) {
  if (supaAvailable() && await isLoggedIn()) {
    try {
      const cartId = await supaEnsureActiveCart();
      await supaRemoveCartItem(cartId, id);
      renderCart();
      return;
    } catch (_) { /* fallback */ }
  }
  const db = cartDB();
  const list = db.read().filter(it => it.id !== id);
  db.write(list);
  renderCart();
}
...
async function cartCount() {
  const list = readLS(LS_KEYS.cart, []);
  return list.reduce((s, it) => s + (it.qty||0), 0);
}

async function cartTotal() {
  const list = readLS(LS_KEYS.cart, []);
  return list.reduce((sum, it) => {
    const p = getItemById(it.id);
    return sum + (p ? p.price * (it.qty||0) : 0);
  }, 0);
}

// ----- Supabase Cart helpers -----
async function supaEnsureActiveCart() {
  const uid = await currentUserId();
  if (!uid) throw new Error('not_logged');
  // find active cart or create
  const { data, error } = await supabase.from('carts').select('id,status').eq('user_id', uid).eq('status','active').limit(1).maybeSingle();
  if (!error && data?.id) return data.id;
  const ins = await supabase.from('carts').insert({ user_id: uid, status: 'active' }).select('id').maybeSingle();
  if (ins.error) throw ins.error;
  return ins.data.id;
}

async function supaGetCartItems(cartId) {
  const { data, error } = await supabase.from('cart_items').select('product_id,qty').eq('cart_id', cartId).limit(1000);
  if (error) throw error;
  return (data||[]).map(r => ({ id: String(r.product_id), qty: Number(r.qty||0) }))
}

async function supaUpsertCartItem(cartId, productId, qty, increment) {
  // try select existing
  const { data } = await supabase.from('cart_items').select('id,qty').eq('cart_id', cartId).eq('product_id', productId).maybeSingle();
  if (data?.id) {
    const newQty = increment ? Number(data.qty||0) + qty : qty;
    await supabase.from('cart_items').update({ qty: newQty }).eq('id', data.id);
  } else {
    await supabase.from('cart_items').insert({ cart_id: cartId, product_id: productId, qty: Math.max(1, qty) });
  }
  // mirror to local cache for badge/total without extra roundtrips
  const list = await supaGetCartItems(cartId);
  writeLS(LS_KEYS.cart, list);
}

async function supaRemoveCartItem(cartId, productId) {
...
// ----- Migration on login -----
async function migrateLocalToSupabaseOnLogin() {
  if (!supaAvailable() || !(await isLoggedIn())) return;
  // Favorites
  try {
    const localFavs = readLS(LS_KEYS.favs, []);
    if (Array.isArray(localFavs) && localFavs.length) {
      for (const pid of localFavs) { try { await supaAddFavSafe(pid); } catch(_){} }
  ...
  try {
    ...
    if (Array.isArray(localCart) && localCart.length) {
      const cartId = await supaEnsureActiveCart();
      for (const it of localCart) { try { await supaUpsertCartItem(cartId, it.id, it.qty, true); } catch(_){} }
      writeLS(LS_KEYS.cart, []);
    }
  } catch (_) {}
}

// Load current Supabase state into local cache for coherent UI on first render
async function supaBootstrapUserState() {
  if (!supaAvailable() || !(await isLoggedIn())) return;
  // Favorites
  try {
    const favs = await supaGetFavsSafe();
    if (Array.isArray(favs)) writeLS(LS_KEYS.favs, favs);
  } catch (_) {}
  // Cart
  try {
    const cartId = await supaEnsureActiveCart();
    const items = await supaGetCartItems(cartId);
    writeLS(LS_KEYS.cart, items);
  } catch (_) {}
  try { renderCartBadge?.(); } catch(_) {}
  try { renderFavsIconStates?.(); } catch(_) {}
}
...
function initChatbot() {
  ...
  async function getUserName(){
    ...
    try { const u = await getSupabaseUser(); const e = u?.email||''; if (e) return e.split('@')[0]; } catch(_){}
    return '';
  }
...
async function renderShop() {
  const grid = $('#shopGrid');
  const giftBar = document.getElementById('giftFilters');
  const favs = new Set(await favsDB().read());
  // Base by category
...
async function renderFavs() {
  const favIds = new Set(await favsDB().read());
...
// Account page: basic renderer and handlers
async function renderAccount() {
  // Wire buttons
  const btnSignIn = document.getElementById('accSignIn');
  const btnSwitch = document.getElementById('accSwitch');
  const logoutBtn = document.getElementById('accLogout');

  if (logoutBtn && !logoutBtn.dataset.wired) {
    logoutBtn.dataset.wired = '1';
    logoutBtn.onclick = async () => {
      try { if (window.supabase) await supabase.auth.signOut(); } catch(_) {}
      setSessionUser(null);
  ...
  if (btnSignIn && !btnSignIn.dataset.wired) {
    btnSignIn.dataset.wired = '1';
    btnSignIn.onclick = () => window.openAccountAuth?.('login');
  }
  if (btnSwitch && !btnSwitch.dataset.wired) {
    btnSwitch.dataset.wired = '1';
    btnSwitch.onclick = async () => {
      try { if (window.supabase) await supabase.auth.signOut(); } catch(_) {}
      setSessionUser(null);
      try { await renderUser(); } catch(_) {}
      if (typeof window.openAccountAuth === 'function') window.openAccountAuth('login');
    };
  }

  // Orders list (from local history for now)
  const wrap = document.getElementById('accOrders');
  if (wrap) {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  $$("[data-action='logout']").forEach(b => b.addEventListener('click', async () => {
    ...
    try { await supabase.auth.signOut(); showToast('Disconnesso'); await renderUser(); location.hash = '#home'; } catch(err){ console.error(err); showToast('Errore logout','error'); }
  }));
  // Account page logout
    ...
    try {
      ...
      if (mode === 'register') {
        ...
        authResponse = await supabase.auth.signUp({
          email,
          password,
          options: { data: { full_name, phone, address, newsletter } }
        });
        if (authResponse.error) throw authResponse.error;
        showToast('Registrazione riuscita. Controlla la tua email.');

      } else { // Login
        authResponse = await supabase.auth.signInWithPassword({ email, password });
        if (authResponse.error) throw authResponse.error;
        if (!authResponse.data.user) throw new Error('Login fallito: utente non trovato.');
        showToast('Accesso effettuato');
      }

      // Sync settings from user metadata on success for both flows
      try {
        const um = authResponse.data.user.user_metadata || {};
        const s = await getSettings();
        const next = { ...s };
        if (um.full_name) next.name = um.full_name;
        if (um.phone) next.phone = um.phone;
        if (um.address) next.address = um.address;
        if (typeof um.newsletter === 'boolean') next.emailNotifs = um.newsletter;
        await setSettings(next);
      } catch (e) {
        console.error('Failed to sync settings on auth:', e);
      }
  ...
  $('#couponForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (!u || !u.email) { $('#couponMsg').textContent = 'Devi accedere per usare un codice sconto.'; openAuth('login'); return; }
    } catch(_) { $('#couponMsg').textContent = 'Devi accedere per usare un codice sconto.'; openAuth('login'); return; }
  ...
  if (couponCodeEl) {
    ...
    couponCodeEl.addEventListener('input', async () => {
      ...
      couponTypeTimer = setTimeout(async () => {
        ...
        try {
          const u = await getSupabaseUser();
          if (!u || !u.email) { if (msg) msg.textContent = 'Accedi per usare i codici sconto'; el.classList.add('invalid'); return; }
        } catch(_) { if (msg) msg.textContent = 'Accedi per usare i codici sconto'; el.classList.add('invalid'); return; }
  ...
  $('#paymentForm').addEventListener('submit', async (e) => {
    ...
    // If Stripe card selected, create checkout session via Supabase Edge Function
    if (payMethod === 'card') {
      try {
        const ok = await startStripeCheckoutFromPending(pending);
        if (!ok) {
          $('#paymentMsg').textContent = 'Errore durante la creazione della sessione di pagamento.';
        }
      } catch (err) {
        console.error(err);
        $('#paymentMsg').textContent = 'Errore durante la creazione della sessione di pagamento.';
    ...
    // Otherwise go to the dedicated local mock payment pages
...
// Create Stripe checkout session via Supabase Edge Function and redirect
async function startStripeCheckoutFromPending(pending) {
  try {
    if (!(window.supabase && supabase.functions)) return false;
    const items = Array.isArray(pending?.items) ? pending.items : [];
    if (!items.length) return false;
    const payload = {
      items: items.map(it => ({ product_id: it.id || it.product_id, qty: Number(it.qty || 1) })),
      customer_email: pending?.email || undefined,
      metadata: {
        order_id: String(pending?.id || ''),
        ship_method: String(pending?.shipMethod || ''),
        coupon: String(pending?.coupon || ''),
      },
    };
    const { data, error } = await supabase.functions.invoke('create-checkout-session', { body: payload });
    if (error) { console.error('invoke error', error); return false; }
    if (!data?.url) { console.error('missing session url', data); return false; }
