...
function maybeReduceMotionOnMobile() {
  try {
    ...
    if (s.effects !== 'minimal') {
      ...
      try { window.supabase = SUPA; } catch(_) {}
...
window.sbCurrentUser = async () => {
  try {
    if (!window.supaReady || !supaReady()) return null;
    // Prefer session for immediate persisted state
...
window.sbSignOut = async () => {
  try { if (supaReady()) await supabase.auth.signOut(); } catch(_) {}
};
...
const PRODUCTS = [
  ...
  { id: 'Playstation Gift Card 10€', name: 'Carta Regalo PlayStation 10€', price: 9.00, old: 10.00, img: 'assets/play10€.png', tag: 'console' },
  { id: 'Playstation Gift Card 20€', name: 'Carta Regalo PlayStation 20€', price: 19.15, old: 20.00, img: 'assets/play20.jpg', tag: 'console' },
  { id: 'Playstation Gift Card 50€', name: 'Carta Regalo PlayStation 50€', price: 47.50, old: 50.00, img: 'assets/play50€.png', tag: 'console' },
